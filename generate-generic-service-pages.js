            ...
            <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-[2fr_1fr_1fr_1fr] gap-12">
                ...
                <div>
                    ...
                    <ul class="flex flex-col gap-[9px]">
                        ...
                        <li class="flex items-center gap-2.5 text-[14px]" style="color:rgba(255,255,255,.5)"><i class="fas fa-envelope text-orange w-4 shrink-0"></i><a href="mailto:thefixwizard@gmail.com" style="color:rgba(255,255,255,.5)">thefixwizard@gmail.com</a></li>
...
const GENERIC_AREA = 'South Jersey & Greater Philadelphia';
const GENERIC_CITY = 'South Jersey';
const GENERIC_STATE = 'NJ';

function genericPage(svc, type) {
  ...
  const hubSlug   = type === 'chimney' ? 'chimney-services-near-cherry-hill' : 'handyman-services-near-cherry-hill';

  // Replace placeholders with generic area text
  const r = s => s ? s
    .replace(/\{cityState\}/g, GENERIC_AREA)
    .replace(/\{city\}/g, GENERIC_CITY)
    .replace(/\{state\}/g, GENERIC_STATE)
    : s;

  const title   = `${svc.name} | Professional Home Services | The Fix Wizard`;
  const metaDesc = content && content.meta_desc
    ? r(content.meta_desc)
    : `Professional ${svc.name.toLowerCase()} by licensed, insured experts. ${svc.desc.slice(0, 90)}. Free estimates. Same-day available. Serving South Jersey & Greater Philadelphia.`;
  ...
  const schema = JSON.stringify({
    ...
    provider: {
      '@type': 'LocalBusiness',
      name: 'The Fix Wizard',
      telephone: '+15513504951',
      url: 'https://thefixwizard.com',
      areaServed: 'South Jersey and Greater Philadelphia'
    }
  });
                ...
                <div class="space-y-10">
                    <div>
                        <h2 class="text-[22px] font-bold text-navy-900 mb-4 flex items-center gap-3">
                            ...
                            ${r(content.h2_why_need) || 'Why Professional Service Matters'}
                        </h2>
...
<head>
    ...
    <link rel="canonical" href="https://thefixwizard.com/${svc.slug}/">
    <title>${title}</title>
...
${navbar(svcParam)}
    ...
    <section class="hero relative flex items-center overflow-hidden pt-[var(--nav-h)] pb-14 rounded-b-[48px]">
        <div class="max-w-site mx-auto px-6 relative z-[2] w-full pt-8">
            ...
            <nav class="flex items-center gap-2 text-[12px] mb-8 flex-wrap" style="color:rgba(255,255,255,.4)">
                ...
                <a href="/services/${type}/" class="hover:text-orange transition-colors" style="color:rgba(255,255,255,.4)">${typeLabel}</a>
                <i class="fas fa-chevron-right text-[9px]"></i>
        ...
        <div class="max-w-site mx-auto px-6">
            <div class="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-12 items-start">
                ...
                <aside class="flex flex-col gap-6 sticky top-[calc(var(--nav-h)+80px)]">
                    ...
                    <!-- Back link -->
                    <a href="/services/${type}/" class="flex items-center gap-2.5 text-[13px] font-semibold text-orange hover:text-orange-dark transition-colors group">
                    ...
                    <div class="rounded-card p-6 text-white" style="background:#091236">
                        <div class="text-[16px] font-bold mb-1">Get a Free Quote</div>
                    ...
                    <!-- Service area links -->
                    <div class="rounded-card border border-slate-100 p-6">
                        <div class="text-[14px] font-bold text-navy-900 mb-3">Find Us Near You</div>
                        <div class="flex flex-col gap-2">
                            <a href="/${svc.slug}-near-cherry-hill/" class="text-[13px] text-slate-500 hover:text-orange transition-colors flex items-center gap-2"><i class="fas fa-location-dot text-orange text-[11px]"></i> Cherry Hill, NJ</a>
                            <a href="/${svc.slug}-near-moorestown/" class="text-[13px] text-slate-500 hover:text-orange transition-colors flex items-center gap-2"><i class="fas fa-location-dot text-orange text-[11px]"></i> Moorestown, NJ</a>
                            <a href="/${svc.slug}-near-haddonfield/" class="text-[13px] text-slate-500 hover:text-orange transition-colors flex items-center gap-2"><i class="fas fa-location-dot text-orange text-[11px]"></i> Haddonfield, NJ</a>
                            <a href="/${svc.slug}-near-philadelphia/" class="text-[13px] text-slate-500 hover:text-orange transition-colors flex items-center gap-2"><i class="fas fa-location-dot text-orange text-[11px]"></i> Philadelphia, PA</a>
                            <a href="/locations/" class="text-[13px] text-slate-400 hover:text-orange transition-colors flex items-center gap-2 mt-1"><i class="fas fa-map-marker-alt text-orange text-[11px]"></i> All Service Areas &rarr;</a>
                        </div>
                    </div>
                </aside>
            </div>
    ...
    <section class="py-20" style="background-color:#091236">
        <div class="max-w-site mx-auto px-6 text-center">
            <h2 class="font-cinzel text-[clamp(22px,3vw,38px)] font-black text-white mb-4">Need ${svc.name}?</h2>
            <p class="text-[16px] mb-10 max-w-[480px] mx-auto" style="color:rgba(255,255,255,.6)">Call now for same-day service in South Jersey and Greater Philadelphia. Free estimates, no obligation.</p>
            ...
            <p class="text-[13px] mt-6" style="color:rgba(255,255,255,.3)">Serving: Cherry Hill · Haddonfield · Voorhees · Moorestown · Philadelphia · West Chester · Wilmington</p>
        </div>
    </section>
