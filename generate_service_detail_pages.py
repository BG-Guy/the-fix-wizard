...
/* ── Pricing box ── */
.price-box{position:relative;overflow:hidden;background:#091236;border-radius:16px;padding:28px 32px;color:#fff}
.price-label{display:inline-flex;align-items:center;gap:6px;background:rgba(255,107,53,.2);color:#f97e4a;padding:5px 13px;border-radius:99px;font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:.07em;margin-bottom:14px}
.price-text{font-size:15px;color:rgba(255,255,255,.72);line-height:1.8}
.price-watermark{position:absolute;right:16px;bottom:4px;color:rgba(255,255,255,.04);font-size:110px;font-weight:900;line-height:1;pointer-events:none;user-select:none;font-family:'Inter',sans-serif}
...
def blog_sections_html(slug, name, loc_name):
    ...
    return f"""
        ...
        <div class="price-box">
            <div class="price-label"><i class="fas fa-tag"></i> Pricing &amp; Estimates</div>
            <p class="price-text">{c["price"]}</p>
            <a href="../#contact" class="btn btn-primary" style="margin-top:20px;display:inline-flex">
                <i class="fas fa-paper-plane"></i> Get a Free Estimate
            </a>
            <div class="price-watermark">$</div>
        </div>"""
